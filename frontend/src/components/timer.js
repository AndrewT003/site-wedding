...
import CountdownTimer from "../static/js/timer";
import '../static/style/timer.css'
...
function Timer(){
    ...
    return(
        <div ref={elementRef} className={`timer-wrapper ${isVisible ? 'visible' : ''}`}>
            {/* Один блок з фоном - таймер + календар */}
            <div className="waiting_for_you_section">
                <div className="waiting_overlay"></div>
                <div className="waiting_content">
                    {/* Заголовок та таймер */}
                    <h1 className={"Waiting_lable"}>Чекаємо вас</h1>
                    <span className="fly-text">Через</span>
                    <CountdownTimer/>
